...
Generate a natural Polish response summarizing these results.`;
  ...
  private buildSystemPrompt(context: VoiceCommandContext): string {
    const containersInfo = context.containers?.map(c => 
      `${c.name} (${c.type}, ID: ${c.container_id})`
    ).join(', ') || '';

    const shelvesInfo = context.shelves?.map(s => 
      `${s.name} (pozycja ${s.position}, ID: ${s.shelf_id})`
    ).join(', ') || '';
